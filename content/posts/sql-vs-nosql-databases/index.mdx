...
> db.collection.find({"position":"Tech Lead"})

Here collection would be replaced by the name of the collections of documents.
...
- If you are developing a application where security is not a issue, or you
    expect that user is not going to perform complex queries such as JOINS, or maybe just you want to have a flexible schema for database, go for **NoSQL** database. That is why database used by EA Sports, is NoSQL.